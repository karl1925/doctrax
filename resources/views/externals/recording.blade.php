...
@section('title', 'Requests for Recording')
...
<div class="min-h-screen bg-slate-50/50 pb-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
            ...
            <div class="space-y-2">
                <nav class="flex mb-2" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-2 text-xs font-medium text-slate-500 uppercase tracking-wider">
                        ...
                        <li class="text-indigo-600 font-bold">INCOMING</li>
                    </ol>
                </nav>
                <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight flex items-center gap-3">
                    <div class="p-2 bg-gray-500 rounded-lg shadow-lg shadow-gray-200">
                ...
                <p class="text-slate-500 font-medium text-sm">
                    @if($search == '' && $priority == '')
                        There {{ $externals->count() > 1 ? 'are' : 'is' }} <span class="text-slate-900 font-bold">{{ $externals->count() }} incoming {{ Str::plural('request', $externals->count()) }}</span> for recording and action.
                    @else
        ...
        @include('partials.external-filters', ['action' => route('externals.recording')])
        <div class="space-y-4">
            ...
            <div class="bg-white border border-slate-200 rounded-xl p-5 shadow-sm hover:border-gray-200 transition-all group">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="text-right">
                        <div class="pb-3">
                            <span class="font-normal text-xs">Received By:</span><span class="font-bold text-xs">{!! $external->creator->name !!}<br><span class="italic font-normal"> {{ $external->created_at->diffForHumans() }}</span></span>
                        </div>
                        <a href="{{ route('externals.recording.verify', $external->id) }}" class="px-6 py-3 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl">
                            Review
                        </a>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 p-20 flex flex-col items-center text-center">
                ...
                <p class="text-slate-500 font-medium max-w-sm mt-3">
                    @if($search == '' && $priority == '')
                        There are no external requests pending recording and forwarding at the moment. Great job staying on top of things!
                    @endif
                </p>
            </div>
            @endforelse
