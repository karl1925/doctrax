        ...
        <div class="space-y-2">
            <nav class="flex mb-2" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-xs font-medium text-slate-500 uppercase tracking-wider">
                    ...
                    <li class="text-indigo-600 font-bold">PERSONNEL</li>
                </ol>
            </nav>
            ...
            <p class="text-slate-500 font-medium text-sm">
                Displaying {{ $users->total() }} team members.
            </p>
        </div>
    </div>

    <form method="GET" action="{{ route('settings.personnel') }}" class="flex mb-2 w-full ml-auto max-w-md">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 border-b border-slate-100">
                        @foreach(['name' => 'Employee', 'status' => 'Status', 'created_at' => 'Joined'] as $key => $label)
                        <th class="px-6 py-4">
                ...
                <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full {{ $user->isOnline() ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>
                                <span class="text-xs font-semibold text-slate-700 uppercase">{{ $user->isOnline() ? 'Online' : 'Offline' }}</span>
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <form action="{{ route('settings.personnel.destroy', $user) }}" method="POST" onsubmit="return confirm('Archive this user?')">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-slate-500 text-sm italic">No personnel found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<script>
    function openCreateModal() {
        // Implementation for the Slide-over logic can be placed here
