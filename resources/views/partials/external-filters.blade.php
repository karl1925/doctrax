<form method="GET" action="{{ $action }}" class="w-full max-w-4xl ml-auto mb-5" id="external-filters-form">
    <div class="flex flex-col lg:flex-row items-stretch lg:items-center gap-4 p-2 bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm rounded-2xl transition-all hover:shadow-md focus-within:ring-1 focus-within:ring-indigo-100">
        ...
        <div class="relative flex-grow group">
            ...
            <input
                ...
                class="block w-full pl-11 pr-4 py-3 bg-transparent border-none text-slate-700 text-sm placeholder-slate-400 focus:ring-0"
            >
        </div>
        ...
        <div class="flex items-center px-2 py-1 bg-slate-50 rounded-xl lg:bg-transparent overflow-x-auto no-scrollbar">
                ...
                @foreach($options as $opt)
                    <label class="relative flex-1">
                        ...
                        <div class="px-4 py-2 text-xs font-semibold rounded-lg cursor-pointer transition-all text-center
                            ...
                            peer-checked:bg-white peer-checked:text-indigo-600 peer-checked:shadow-sm peer-checked:ring-1 peer-checked:ring-slate-200/60
                            @if($opt['val'] == 'urgent') peer-checked:text-rose-600 @endif
                            @if($opt['val'] == 'high') peer-checked:text-orange-600 @endif">
                            {{ $opt['label'] }}
                        </div>
                    </label>
                @endforeach
